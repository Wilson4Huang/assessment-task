...
# 迭代次数
NUM_ITER = 10000   #调参从2000到10000，loss越来越小，准确率越来越高
# 学习率
LR = 0.7
# #学习率衰减率
# alpha=0.5
...
for i in range(len(set(y_train))):
    TP_i=np.sum(np.logical_and(np.equal(y_train,i),np.equal(y_train_predict,i)))     #同时满足正类的数量
    precision_i=TP_i/np.sum(np.equal(y_train_predict,i))     #精确率：预测为正类的样本中，真正为正类的样本占多少
    recall_i=TP_i/np.sum(np.equal(y_train,i))   #召回率：真实为正类的样本中，预测为正类的样本占多少
...
#分别计算第i类的p，r（第i类为正，其余都为负类），然后进行平均
Precision_mean=Precision_sum/len(set(y_train))
Recall_mean=Recall_sum/len(set(y_train))
Macro_F1score=(2*Precision_mean*Recall_mean)/(Recall_mean+Precision_mean) ##2pr/(p+r)


# In[32]:


Macro_F1score   #多分类评价指标
